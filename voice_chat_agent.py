...
class VoiceChatAgent:
    ...
    def detect_speech(self, audio_chunk: np.ndarray) -> bool:
        ...
        # If chunk is too short, pad with zeros
            ...
            audio_chunk = np.pad(audio_chunk, (0, expected_samples - len(audio_chunk)), 'constant')
        
        # Convert to tensor and ensure correct format
        audio_tensor = torch.FloatTensor(audio_chunk).unsqueeze(0)  # Add batch dimension
    ...
    def _process_audio_loop(self):
        """Process audio chunks for VAD and speech recognition."""
        vad_buffer = np.array([])  # Buffer for VAD processing
            ...
            try:
                ...
                while len(vad_buffer) >= self.vad_chunk_size:
                    ...
                    if has_speech:
                        ...
                        # Add the original audio chunk to buffer (not the VAD chunk)
